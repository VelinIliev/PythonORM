...
def product_quantity_ordered():
    ...
    return '\n'.join(f'Quantity ordered of {p["product__name"]}: {p["total_ordered_quantity"]}'
...
def ordered_products_per_customer():
    ...
    for order in orders:
        ...
        [output.append(f'- Product: {op.product.name}, Category: {op.product.category.name}') for op in products]
...
def filter_products():
    ...
    output = [f"{p.name}: {p.price}lv." for p in products]
    return '\n'.join(output)
...
def give_discount():
    products_to_discount = Product.objects.filter(price__gt=3)
