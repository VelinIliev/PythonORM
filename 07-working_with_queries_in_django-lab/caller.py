...
def delete_review_by_id(review_id):
    review = Review.objects.filter(id=review_id).get()
    output = f'Review by {review.reviewer_name} was deleted'
    review.delete()
    return output
...
def filter_authors_by_nationalities(nationality):
    ...
    for author in authors:
        if author.biography:
            output.append(author.biography)
        else:
            output.append(f'{author.first_name} {author.last_name}')
    return '\n'.join(output)
...
def filter_authors_by_birth_year(year_one, year_two):
    authors = Author.objects.filter(
        birth_date__year__gte=year_one,
        birth_date__year__lte=year_two) \
        .order_by('-birth_date')
